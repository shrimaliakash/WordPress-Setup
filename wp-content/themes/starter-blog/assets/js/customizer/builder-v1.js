		...
		var Builder = {
			...
			drag_drop: function() {
				...
				$(".starterblog--device-panel", that.container).each(function() {
					...
					var sortable_ids = [];
					that.panels[device] = {};
					$(".starterblog--cb-items", panel).each(function(index) {
					...
					$(
						".starterblog-available-items .grid-stack-item",
						panel
					).draggable({
						revert: "invalid",
						...
						start: function(event, ui) {
							...
							$(".starterblog--cb-items", panel).css("z-index", "");
			...
			addNewWidget: function($item, row) {
				...
				var panel = that.container.find(
					".starterblog--device-panel.starterblog--panel-" +
						that.activePanel
				);
				var el = row;
				if (!_.isObject(el)) {
					el = panel.find(".starterblog--cb-items").first();
				}
			...
			addPanel: function(device) {
				...
				var template_id = "tmpl-starterblog--cb-panel";
			...
			addAvailableItems: function() {
				...
				_.each(that.devices, function(device_name, device) {
					var $itemWrapper = $(
						'<div class="starterblog-available-items" data-device="' +
							device +
							'"></div>'
					);
					...
					_.each(device_data, function(items, row_id) {
						if (!_.isUndefined(items)) {
							_.each(items, function(node, index) {
								var item = $(
									'.starterblog-available-items[data-device="' +
										device +
