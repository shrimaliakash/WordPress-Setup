(function($, wpcustomize) {
	...
	wpcustomize.bind("ready", function(e, b) {
		var addVersionChange = function(opts, builder, id, version) {
			...
			if (typeof opts.versions === "object") {
				...
				if (select_options) {
					var $select_options = $(
						'<span class="starterblog-swicth-version button-primary"><span>'+StarterBlog_Layout_Builder.swicth_version+'</span><select class="builder-version-change ">' +
							select_options +
							"</select></span>"
					);
					$(".starterblog--cb-actions", builder.container).prepend(
						$select_options
					);
				}
				...
				$select_options.on("change", 'select', function() {
					var newVer = $(this).val();
					if ( starterblogPanels[id + version] ) {
						starterblogPanels[id + version].container.hide();
					}
					
					if (typeof starterblogPanels[id + newVer] === "undefined") {
						var _builder;
						if ("v2" === newVer) {
						...
						starterblogPanels[id + newVer] = _builder;
					}
					...
					if ( starterblogPanels[id + newVer] ) {
						starterblogPanels[id + newVer].container.show();
					}
		...
		_.each(StarterBlog_Layout_Builder.builders, function(opts, id) {
			var builder;
			var version = "v1";
			...
			starterblogPanels[id + version] = builder;
				...
				if (
					...
					$(
						...
						'<p class="footer_moved_widgets_text">' +
							StarterBlog_Layout_Builder.footer_moved_widgets_text +
							"</p>"
					);
				}
			}, 1000);
		...
		wpcustomize.state("expandedSection").bind(function(section) {
			$(".starterblog--device-panel .grid-stack-item").removeClass(
				"item-active"
			);
			$(".starterblog--cb-row").removeClass("row-active");
			if (section) {
				$('.starterblog--cb-row[data-id="' + section.id + '"]').addClass(
					"row-active"
				);
				$(
					".starterblog--device-panel .grid-stack-item.for-s-" +
						section.id
		...
		function(e) {
			...
			if (template_name && template_name !== "") {
				...
				$.post(
					ajaxurl,
					{
						action: "starterblog_builder_save_template",
						name: input.val(),
	...
	$document.on("mouseover", ".starterblog--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
		var item = $(this);
		var nameW =
			$(".starterblog--cb-item-remove", item).outerWidth() +
			$(".starterblog--cb-item-setting", item).outerWidth();
	...
	$document.on("mouseleave", ".starterblog--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
