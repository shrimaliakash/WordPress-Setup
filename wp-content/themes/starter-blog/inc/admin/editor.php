 ...
 * @since 1.5.0
 */
class StarterBlog_Editor {
	private $action = 'starterblog_load_editor_style';
	 ...
	 */
	function assets() {
		wp_enqueue_style( 'starterblog-editor-fonts', StarterBlog_Customizer_Auto_CSS::get_instance()->get_font_url() );
	...
	public function css() {
		...
		foreach ( $keys as $k ) {
			$f  = StarterBlog()->customizer->get_field_setting( $k );
			if ( $f ) {
				$fields[ $k ] = $f;
			}
		}
		...
		$c = new StarterBlog_Customizer_Auto_CSS();
