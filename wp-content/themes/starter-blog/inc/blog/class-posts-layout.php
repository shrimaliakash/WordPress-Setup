...
class StarterBlog_Posts_Layout {
	public $args = array();
	...
	function set_args( $customizer_args = array() ) {
		$args = array(
			'layout'              => StarterBlog()->get_setting( $customizer_args['prefix'] . '_layout' ),
			'excerpt_type'        => StarterBlog()->get_setting( $customizer_args['prefix'] . '_excerpt_type' ),
			'excerpt_length'      => StarterBlog()->get_setting( $customizer_args['prefix'] . '_excerpt_length' ),
			'excerpt_more'        => StarterBlog()->get_setting( $customizer_args['prefix'] . '_excerpt_more' ),
			'more_text'           => StarterBlog()->get_setting( $customizer_args['prefix'] . '_more_text' ),
			'more_display'        => StarterBlog()->get_setting( $customizer_args['prefix'] . '_more_display' ),
			'thumbnail_size'      => StarterBlog()->get_setting( $customizer_args['prefix'] . '_thumbnail_size' ),
			'hide_thumb_if_empty' => StarterBlog()->get_setting( $customizer_args['prefix'] . '_hide_thumb_if_empty' ),
			'meta_config'         => StarterBlog()->get_setting( $customizer_args['prefix'] . '_meta_config' ),
			'meta_sep'            => StarterBlog()->get_setting( $customizer_args['prefix'] . '_meta_sep' ),
			'author_avatar'       => StarterBlog()->get_setting( $customizer_args['prefix'] . '_author_avatar' ),
			'media_hide'          => StarterBlog()->get_setting( $customizer_args['prefix'] . '_media_hide' ),
		);

		$size = StarterBlog()->get_setting( $customizer_args['prefix'] . '_avatar_size' );
		if ( is_array( $size ) && isset( $size['value'] ) ) {
			$args['avatar_size'] = absint( $size['value'] );
		}

		$pagination = array(
			'show_paging' => StarterBlog()->get_setting( $customizer_args['prefix'] . '_pg_show_paging' ),
			'show_nav'    => StarterBlog()->get_setting( $customizer_args['prefix'] . '_pg_show_nav' ),
			'mid_size'    => StarterBlog()->get_setting( $customizer_args['prefix'] . '_pg_mid_size' ),
			'prev_text'   => StarterBlog()->get_setting( $customizer_args['prefix'] . '_pg_prev_text' ),
			'next_text'   => StarterBlog()->get_setting( $customizer_args['prefix'] . '_pg_next_text' ),
		);
		...
		StarterBlog_Post_Entry()->set_config( $this->args );
	}
	...
	function item_part( $part = '', $post = null, $inner_class = '' ) {
		...
		if ( isset( $this->args[ $part . '_fields' ] ) && $n > 0 ) {
			if ( 'media' == $part && $this->has_only_field( $this->args[ $part . '_fields' ] ) ) {
				StarterBlog_Post_Entry()->build_fields( $this->args[ $part . '_fields' ], $post );
			} else {
			...
			case 'blog_column':
				...
				if ( $show_media && $this->count_item_visibility( $this->args['header_fields'] ) ) {
					...
					<div class="entry-article-part entry-media">
						...
						<?php
						StarterBlog_Post_Entry()->build_fields( $media_fields, $post );
	...
	function blog_item( $post = null, $class = null ) {
		...
		StarterBlog_Post_Entry()->set_post( $post );
		/**
		 * Hook before each post
		 *
		 * @since 1.5.0
		 */
		do_action( "starterblog/before-post/{$key}" );
		?>
		 ...
		 * @since 1.5.0
		 */
		do_action( "starterblog/after-post/{$key}" );
	}
	...
	function render( $customizer_args = array() ) {
		...
		$atts = array();
		if ( is_numeric( $this->args['columns'] ) && $this->args['columns'] > 1 ) {
			$classes['grid']  = 'starterblog-grid-' . $this->args['columns'];
			...
			$classes['grid'] = sprintf( 'starterblog-grid-%1$s_sm-%2$s_xs-%3$s', $this->args['columns']['desktop'], $this->args['columns']['tablet'], $this->args['columns']['mobile'] );
		}
		...
		do_action( 'starterblog/blog/before-render', $this );
		?>
		...
		<?php
		do_action( 'starterblog/blog/after-render', $this );
	}
	...
	function render_pagination() {
		...
		if ( $this->args['pagination']['show_nav'] ) {
			...
			if ( ! $prev_text ) {
				$prev_text = _x( 'Previous', 'previous set of posts', 'starter-blog' );
			}
			if ( ! $next_text ) {
				$next_text = _x( 'Next', 'next set of posts', 'starter-blog' );
			}
		} else {
			$prev_text = false;
			$next_text = false;
