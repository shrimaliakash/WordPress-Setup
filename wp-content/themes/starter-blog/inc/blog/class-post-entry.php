...
class StarterBlog_Post_Entry {
	...
	function get_config_default() {
		$args = array(
			'excerpt_type'   => 'custom',
			'excerpt_length' => StarterBlog()->get_setting( 'blog_post_excerpt_length' ),
			'excerpt_more'   => null,
			'thumbnail_size' => StarterBlog()->get_setting( 'blog_post_thumb_size' ),
			'meta_config'    => StarterBlog()->get_setting( 'blog_post_meta' ),
			'meta_sep'       => _x( '-', 'post meta separator', 'starter-blog' ),
			'more_text'      => null,
			'more_display'   => 1,
			'term_sep'       => _x( ',', 'post term separator', 'starter-blog' ),
			'term_count'     => 1,
			'tax'            => 'category',
			'title_tag'      => 'h2',
			'title_link'     => 1,
			'author_avatar'  => StarterBlog()->get_setting( 'blog_post_author_avatar' ),
			'avatar_size'    => 32,
		);

		$size = StarterBlog()->get_setting( 'blog_post_avatar_size' );
	...
	function meta_tags() {
		...
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'starter-blog' ) );
			if ( $tags_list ) {
	...
	function post_tags() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'starter-blog' ) );
			if ( $tags_list ) {
				$html .= sprintf( '<div class="entry--item entry-tags tags-links">' . esc_html__( 'Tagged ', 'starter-blog' ) . '%1$s</div>', $tags_list ); // WPCS: XSS OK.
			}
		}
		echo $html;
	}
	...
	function post_categories() {
		...
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$list = get_the_category_list( esc_html_x( ', ', 'list item separator', 'starter-blog' ) );
			if ( $list ) {
				$html .= sprintf( '<div class="entry--item entry-categories cats-links">' . esc_html__( 'Posted in ', 'starter-blog' ) . '%1$s</div>', $list ); // WPCS: XSS OK.
			}
		}
		echo $html;
	}
	...
	function meta_comment() {
		...
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			if ( 1 === $comment_count ) {
				$html .= sprintf(
					/* translators: 1: title. */
					esc_html__( '1 Comment', 'starter-blog' ),
					$comment_count
				);
			} else {
				$html .= sprintf( // WPCS: XSS OK.
					/* translators: 1: comment count number, 2: title. */
					esc_html( _nx( '%1$s Comment', '%1$s Comments', $comment_count, 'comments number', 'starter-blog' ) ),
	...
	private function show_post_meta( $post ) {
		return apply_filters( 'starterblog/show/post_meta', get_post_type( $post ) == 'post' && ! is_search() );
	}
	...
	function post_readmore() {
		...
		if ( ! $more ) {
			if ( ! is_rtl() ) {
				$more = __( 'Read more &rarr;', 'starter-blog' );
			} else {
				$more = __( 'Read more &larr;', 'starter-blog' );
			}
		}
		?>
		<div class="entry-readmore entry--item">
			<a class="readmore-button" href="<?php the_permalink(); ?>" title="<?php esc_attr( sprintf( __( 'Continue reading %s', 'starter-blog' ), get_the_title() ) ); ?>"><?php echo wp_kses_post( $more ); ?></a>
		</div><!-- .entry-content -->
		<?php
	}
		...
		if ( is_single() ) {
			...
			if ( is_single() && isset( $post->post_author ) ) {
				...
				if ( ! empty( $display_name ) ) {
					$author_details = '<h4 class="author-bio-heading">' . sprintf( __( 'About the Author: <span>%s</span>', 'starter-blog' ), $display_name ) . '</h4>';
				}
		...
		the_post_navigation(
			array(
				'next_text' => '<span class="meta-nav text-uppercase text-xsmall color-meta" aria-hidden="true">'
								. __( 'Next', 'starter-blog' ) . '</span> '
								. '<span class="screen-reader-text">' . __( 'Next post:', 'starter-blog' ) . '</span> '
				...
				'prev_text' => '<span class="meta-nav text-uppercase text-xsmall color-meta" aria-hidden="true">'
								. __( 'Previous', 'starter-blog' ) . '</span> '
								. '<span class="screen-reader-text">' . __( 'Previous post:', 'starter-blog' ) . '</span> '
								. '<span class="post-title text-large">%title</span>',
			)
		);
		echo '</div>';
	...
	function post_related() {
		...
		StarterBlog_Related_Posts::get_instance()->display();
	}
	...
	function build( $field, $post = null, $fields = null, $args = array() ) {
		...
		$cb = apply_filters( 'starterblog/single/build_field_callback', false, $field );
		if ( ! is_callable( $cb ) ) {
		 ...
		 * @since 1.5.0
		 */
		do_action( "starterblog/{$type}/field_{$field}/before", $post, $fields, $args, $this );
		if ( is_callable( $cb ) ) {
		 ...
		 * @since 1.5.0
		 */
		do_action( "starterblog/{$type}/field_{$field}/after", $post, $fields, $args, $this );
	}
