 ...
 * @since 1.5.0
 */
class StarterBlog_Layout_Builder_Frontend extends StarterBlog_Abstract_Layout_Frontend {
	...
	function get_settings() {
		...
		$data = StarterBlog()->get_setting( $this->control_id );
		$data = wp_parse_args(
			$data,
			array(
				'desktop' => '',
				...
				if ( is_array( $row_items ) && count( $row_items ) ) {
					...
					foreach ( $row_items as $item_index => $item ) {
						...
						if ( ! $items[ $item['id'] ] ['render_content'] ) {
							 ...
							 * @since 1.5.0
							 */
							do_action( 'starterblog/builder/' . $this->id . '/before-item/' . $item['id'] );
							$object_item = StarterBlog_Customize_Layout_Builder()->get_builder_item( $this->id, $item['id'] );
							...
							if ( ! $has_cb ) {
								...
								$fn            = 'starterblog_builder_' . $id . '_item';
								$return_render = false;
								if ( function_exists( $fn ) ) {
									$return_render = call_user_func_array( $fn, array( $item_config, $item ) );
									$has_cb        = true;
								} else {
									$fn = 'starterblog_builder_' . $this->id . '_' . $id . '_item';
									if ( function_exists( $fn ) ) {
										$return_render = call_user_func_array( $fn, array( $item_config, $item ) );
										$has_cb        = true;
									}
							 ...
							 * @since 1.5.0
							 */
							do_action( 'starterblog/builder/' . $this->id . '/after-item/' . $item['id'] );
	...
	public function render_row( $items, $id = '', $device = 'desktop' ) {
		...
		while ( $index < $n ) {
			...
			$merge      = StarterBlog()->get_setting( $merge_key, $device );
			$merge_next = false;
			$merge_prev = false;
			if ( 'no' == $merge || '0' == $merge ) {
				$merge = false;
			...
			if ( $next_item ) {
				...
				$merge_next     = StarterBlog()->get_setting( $merge_key_next, $device );
			}
		...
		foreach ( $group_items as $item ) {
			...
			if ( 'footer' != $this->id ) {
				$classes[] = "starterblog-col-{$width}_md-{$width}_sm-{$width}";
			} else {
				if ( $number_group_item > 1 ) {
					$classes[] = "starterblog-col-{$width}_md-{$width}_sm-6_xs-12";
				} else {
					$classes[] = "starterblog-col-{$width}_md-{$width}_sm-12_xs-12";
				}
			}
			...
			$classes = apply_filters( 'starterblog/builder/item-wrapper-classes', $classes, $item );
	...
	public function render( $row_ids = array( 'top', 'main', 'bottom' ) ) {
		...
		foreach ( $row_ids as $row_id ) {
			$show = starterblog_is_builder_row_display( $this->id, $row_id );
			if ( $show && isset( $this->rows[ $row_id ] ) ) {
				...
				if ( ! empty( $show_on_devices ) ) {
					...
					if ( ! empty( $desktop_items ) || ! empty( $mobile_items ) ) {
						...
						$align_classes = 'starterblog-grid-middle';
						if ( 'footer' != $this->id ) {
							...
							$align_classes = 'starterblog-grid-top';
						}

						$row_layout    = StarterBlog()->get_setting( $this->id . '_' . $row_id . '_layout' );
						$row_text_mode = StarterBlog()->get_setting( $this->id . '_' . $row_id . '_text_mode' );
						if ( $row_layout ) {
							$classes[] = sanitize_text_field( $row_layout );
						}

						$classes = apply_filters( 'starterblog/builder/row-classes', $classes, $row_id, $this );

						$atts['class']       = join( ' ', $classes );
						$atts['id']          = 'cb-row--' . $_id;
						$atts['data-row-id'] = $row_id;
						$atts                = apply_filters( 'starterblog/builder/row-attrs', $atts, $row_id, $this );
						$string_atts         = '';
						foreach ( $atts as $k => $s ) {
							if ( is_array( $s ) ) {
								$s = wp_json_encode( $s );
						...
						$inner_class = apply_filters( 'starterblog/builder/inner-row-classes', $inner_class, $row_id, $this );
									...
									<div class="starterblog-container">
										...
										if ( $html_desktop ) {
											...
											if ( $html_desktop ) {
												...
												echo '<div class="starterblog-grid ' . esc_attr( $c . ' ' . $align_classes ) . '">';
												echo $html_desktop;
												echo '</div>';
											}
										}
										...
										if ( $html_mobile ) {
											echo '<div class="cb-row--mobile hide-on-desktop starterblog-grid ' . esc_attr( $align_classes ) . '">';
											echo $html_mobile;
											echo '</div>';
										}
										?>
	...
	public function render_mobile_sidebar() {
		...
		$menu_sidebar_skin = StarterBlog()->get_setting( 'header_sidebar_skin_mode' );
 ...
 * @deprecated 0.2.7
 */
class StarterBlog_Customize_Layout_Builder_Frontend extends StarterBlog_Layout_Builder_Frontend {}

/**
 * Alias of class StarterBlog_Layout_Builder_Frontend
 *
 * @see StarterBlog_Layout_Builder_Frontend
 *
 * @return StarterBlog_Layout_Builder_Frontend
 */
function StarterBlog_Layout_Builder_Frontend() {
	return StarterBlog_Layout_Builder_Frontend::get_instance();
}
 ...
 * @return StarterBlog_Layout_Builder_Frontend
 */
function StarterBlog_Customize_Layout_Builder_Frontend() {
	return StarterBlog_Layout_Builder_Frontend();
}
