 ...
 *
 * Class StarterBlog_Customize_Layout_Builder
 */
class StarterBlog_Customize_Layout_Builder {
	static $_instance;
	...
	function init() {
		...
		do_action( 'starterblog/customize-builder/init' );

		if ( is_admin() ) {
			add_action( 'customize_controls_enqueue_scripts', array( $this, 'scripts' ) );
			add_action( 'customize_controls_print_footer_scripts', array( $this, 'template' ) );
			add_action( 'wp_ajax_starterblog_builder_save_template', array( $this, 'ajax_save_template' ) );
			add_action( 'wp_ajax_starterblog_builder_export_template', array( $this, 'ajax_export_template' ) );
		}
	 ...
	 *
	 * @see StarterBlog_Customize_Builder_Panel
	 *
	 * @param string                            $id    ID of panel.
	 * @param StarterBlog_Customize_Builder_Panel $class Panel class name.
	 *
	 * @return bool
	 */
	public function register_builder( $id, $class ) {
		...
		if ( ! $class instanceof StarterBlog_Customize_Builder_Panel ) {
			$name = get_class( $class );
			_doing_it_wrong( $name, sprintf( __( 'Class <strong>%s</strong> do not extends class <strong>StarterBlog_Customize_Builder_Panel</strong>.', 'starter-blog' ), $name ), '1.0.0' );
			return false;
		}

		add_filter( 'starterblog/customizer/config', array( $class, '_customize' ), 35, 2 );
	 ...
	 *
	 * @see StarterBlog_Customize_Layout_Builder::register_builder();
	 *
	...
	function get_builder_items( $builder_id ) {
		if ( ! $builder_id ) {
			return apply_filters( 'starterblog/builder/' . $builder_id . '/items', array() );
		}
		if ( ! isset( $this->registered_items[ $builder_id ] ) ) {
			return apply_filters( 'starterblog/builder/' . $builder_id . '/items', array() );
		}
		...
		$items = apply_filters( 'starterblog/builder/' . $builder_id . '/items', $items );
	...
	function ajax_save_template() {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( __( 'Access denied', 'starter-blog' ) );
		}
		...
		if ( ! $save_name ) {
			$save_name = sprintf( __( 'Saved %s', 'starter-blog' ), date_i18n( 'Y-m-d H:i:s' ) );
		}
		$fn = false;
		if ( ! isset( $this->registered_builders[ $id ] ) ) {
			wp_send_json_error( __( 'No Support', 'starter-blog' ) );
		} else {
		...
		if ( ! $save_name ) {
			...
			$save_name = sprintf( __( 'Saved %s', 'starter-blog' ), $key_id );
		} else {
			$key_id = $save_name;
		}
		...
		update_option( $option_name, $saved_templates );
		$html = '<li class="saved_template li-boxed" data-control-id="' . esc_attr( $control ) . '" data-id="' . esc_attr( $key_id ) . '" data-data="' . esc_attr( wp_json_encode( $new_template_data ) ) . '">' . esc_html( $save_name ) . ' <a href="#" class="load-tpl">' . __( 'Load', 'starter-blog' ) . '</a><a href="#" class="remove-tpl">' . __( 'Remove', 'starter-blog' ) . '</a></li>'; // WPCS: XSS OK.
		wp_send_json_success(
			array(
				'key_id' => $key_id,
				'name'   => $save_name,
	...
	public function get_builders() {
		...
		foreach ( $this->registered_builders as $id => $builder ) {
			...
			$config['items'] = apply_filters( 'starterblog/builder/' . $id . '/items', $this->get_builder_items( $id ) );
			$config['rows']  = apply_filters( 'starterblog/builder/' . $id . '/rows', $builder->get_rows_config() );
			$builders[ $id ] = $config;
		}
	...
	function scripts() {
		...
		wp_localize_script(
			'jquery',
			'StarterBlog_Layout_Builder',
			array(
				'footer_moved_widgets_text' => '',
				'builders'                  => $this->get_builders(),
				'is_rtl'                    => is_rtl(),
				'change_version_nonce'      => wp_create_nonce( 'change_version_nonce' ),
				'swicth_version'            => __( 'Switch Builder Version', 'starter-blog' ),
				'hide_switcher'             => apply_filters( 'starterblog_hide_header_builder_switcher', 'no' ), // Use get theme mod `hide_header_builder_switcher` for hide switcher.
	...
	function template() {
		...
		?>
		<script type="text/html" id="tmpl-starterblog--builder-panel">
			<div class="starterblog--customize-builder">
				<div class="starterblog--cb-inner">
					<div class="starterblog--cb-header">
						<div class="starterblog--cb-devices-switcher">
						</div>
						<div class="starterblog--cb-actions">
							<?php do_action( 'starterblog/builder-panel/actions-buttons' ); ?>
							<a data-id="{{ data.id }}_templates" class="focus-section button button-secondary" href="#"><?php _e( 'Templates', 'starter-blog' ); ?></a>
							<a class="button button-secondary starterblog--panel-close" href="#">
								<span class="close-text"><?php _e( 'Close', 'starter-blog' ); ?></span>
								<span class="panel-name-text">{{ data.title }}</span>
							</a>
						</div>
					</div>
					<div class="starterblog--cb-body"></div>
				</div>
			</div>
		</script>


		<script type="text/html" id="tmpl-starterblog--cb-item">
			...
			>
				...
				<div class="grid-stack-item-content">
					<span class="starterblog--cb-item-name" data-section="{{ data.section }}">{{ data.name }}</span>
					<span class="starterblog--cb-item-remove starterblog-cb-icon"></span>
					<span class="starterblog--cb-item-setting starterblog-cb-icon" data-section="{{ data.section }}"></span>
				</div>
			</div>
		</script>

		<?php
		if ( ! apply_filters( 'starterblog/is_pro_activated', false ) ) {
			?>
			<script type="text/html" id="starterblog-upsell-tmpl">
				<p class="starterblog-upsell-panel"><?php _e( 'Enjoy building? Upgrade to <a target="_blank" href="https://fancywp.com/downloads/starter-blog-pro/?utm_source=theme_dashboard&utm_medium=links&utm_campaign=panel_text">Starter Blog Pro</a> to get more builder items and other premium features</a>.', 'starter-blog' ); ?></p>
			</script>
			<?php
		}
	}
...
/**
 * Alias of class StarterBlog_Customize_Layout_Builder
 *
 * @see StarterBlog_Customize_Layout_Builder
 *
 * @return StarterBlog_Customize_Layout_Builder
 */
function StarterBlog_Customize_Layout_Builder() {
	return StarterBlog_Customize_Layout_Builder::get_instance();
}
