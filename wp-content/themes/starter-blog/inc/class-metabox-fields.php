...
class StarterBlog_Form_Fields {
	 ...
	 */
	private $group_name = 'starterblog_page_settings';
	...
	private function render_fields( $tab_id = false ) {
		foreach ( $this->fields as $field ) {
			...
			$cb        = apply_filters( 'starterblog_render_field_cb', false, $field );
			$_in_class = false;
			if ( ! is_callable( $cb ) ) {
			...
			if ( $content ) {
				...
				echo '<div class="starterblog-mt-field-inner">';
	...
	private function render_tabs() {
		if ( $this->using_tabs && ! empty( $this->tabs ) ) {
			echo '<div class="starterblog-mt-tabs">';
			echo '<ul class="starterblog-mt-tabs-list">';
			$i = 0;
			foreach ( $this->tabs as $id => $tab ) {
				$icon  = '';
				$class = ' starterblog-mt-tab-cont';
				if ( 0 == $i ) {
					$class .= ' active ';
				}
			...
			echo '<div class="starterblog-mt-tab-contents">';
			$i = 0;
			foreach ( $this->tabs as $id => $tab ) {
				$class = 'starterblog-mt-tab-cont';
				if ( 0 == $i ) {
	...
	public function render() {
		if ( $this->using_tabs ) {
			...
			$class = 'starterblog-mt-tabs starterblog-mt-box';
			echo '<div class="' . $class . '">';
			echo '<div class="starterblog-mt-box-inner">';
			$this->render_fields();
			echo '</div>';
			echo '</div>';
		}
	...
	function before_field( $args ) {
		echo "<div class=\"starterblog-mt-field field-type-{$args['type']}\">";
	}
	...
	private function get_filed_id( $args ) {
		...
		return 'starterblog-mt-field-' . $key;
	}
	...
	public function label( $args ) {
		if ( $args['title'] ) {
			?>
			<label class="starterblog-mt-field-label" for="<?php echo esc_attr( $this->get_filed_id( $args ) ); ?>"><?php echo $args['title']; // WPCS: XSS OK. ?></label>
			<?php
		}
	}
	...
	private function field_select( $args ) {
		?>
		<select id="starterblog-met-field-<?php echo esc_attr( $args['name'] ); ?>" name="<?php echo esc_attr( $this->get_name( $args ) ); ?>">
			<?php if ( $args['show_default'] ) { ?>
				<option value=""><?php echo $args['default_label'] ? $args['default_label'] : __( 'Default', 'starter-blog' ); ?></option>
			<?php } ?>
	...
	public function field_image( $args ) {
		...
		$args = wp_parse_args(
			$args,
			array(
				'select_label' => __( 'Select image', 'starter-blog' ),
				'remove_label' => __( 'Remove', 'starter-blog' ),
			)
		);

		$img = StarterBlog()->get_media( $image );
		?>
		<span class="starterblog-mt-media <?php echo ( $img ) ? 'attachment-added' : 'no-attachment'; ?>">
			<span class="starterblog-image-preview">
				<?php
				if ( $img ) {
			...
			<a class="starterblog--add" href="#"><?php echo $args['select_label'];// WPCS: XSS OK. ?></a>
			<a class="starterblog--remove" href="#"><?php echo $args['remove_label'];// WPCS: XSS OK. ?></a>
