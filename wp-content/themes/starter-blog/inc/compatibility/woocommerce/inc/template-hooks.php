 ...
 */
function starterblog_wc_reset_default_hooks() {
	...
	if ( StarterBlog()->get_setting( 'wc_single_layout_breadcrumb' ) ) {
		add_action( 'woocommerce_single_product_summary_before', 'woocommerce_breadcrumb', 5 );
	}
	add_action( 'starterblog/wc-product/before-media', 'woocommerce_template_loop_product_link_open', 10 );
	add_action( 'starterblog/wc-product/after-media', 'woocommerce_template_loop_product_link_close', 10 );
}

add_action( 'wp', 'starterblog_wc_reset_default_hooks' );


/**
 * Display secondary thumbnail.
 */
function starterblog_wc_secondary_product_thumbnail() {
 ...
 */
add_action( 'woocommerce_before_shop_loop', 'starterblog_wc_catalog_header', 15 );
 ...
 * @return bool
 */
function starterblog_wc_catalog_header() {
	...
	if ( ! StarterBlog()->get_setting( 'wc_cd_show_catalog_header' ) ) {
		return false;
	}
	echo '<div class="wc-catalog-header">';
	starterblog_wc_catalog_view_mod();
 ...
 * @return string
 */
function starterblog_wc_catalog_view_mod() {
	if ( ! StarterBlog()->get_setting( 'wc_cd_show_view_mod' ) ) {
		return '';
	}

	$default = starterblog_get_default_catalog_view_mod();
	?>
 ...
 * Loop Layout.
 */
add_action( 'woocommerce_before_subcategory', 'starterblog_wc_before_shop_loop_item', 1 );
add_action( 'woocommerce_after_subcategory', 'starterblog_wc_after_shop_loop_item', 9999 );
add_filter( 'woocommerce_after_output_product_categories', 'starterblog_wc_after_output_product_categories' );

function starterblog_wc_before_shop_loop_item() {
 ...
 * After loop layout
 */
function starterblog_wc_after_shop_loop_item() {
	echo '</div>';
}
 ...
 * @return string
 */
function starterblog_wc_after_output_product_categories( $html ) {
 ...
 */
function starterblog_your_order_heading() {
	?>
	<h3 class="order_review_heading"><?php _e( 'Your order', 'starter-blog' ); ?></h3>
	<?php
}

add_action( 'woocommerce_checkout_order_review', 'starterblog_your_order_heading', 1 );
