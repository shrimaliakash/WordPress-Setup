...
class  StarterBlog_Customizer {
	...
	function init() {
		...
		if ( is_admin() || is_customize_preview() ) {
			...
			add_action( 'wp_ajax_starterblog/customizer/ajax/get_icons', array( $this, 'get_icons' ) );
		...
		add_action( 'wp_ajax_starterblog__reset_section', array( 'StarterBlog_Customizer', 'reset_customize_section' ) );
	}

	/**
	 * Instance.
	 *
	 * @return StarterBlog_Customizer
	 */
	...
	function get_icons() {
		...
		wp_send_json_success( StarterBlog_Font_Icons::get_instance()->get_icons() );
		die();
	}
	...
	function preview_js() {
		if ( is_customize_preview() ) {
			...
			wp_enqueue_script( 'starterblog-customizer-auto-css', esc_url( get_template_directory_uri() ) . '/assets/js/customizer/auto-css' . $suffix . '.js', array( 'customize-preview' ), '20151215', true );
			wp_enqueue_script(
				'starterblog-customizer',
			...
			wp_localize_script(
				'starterblog-customizer-auto-css',
				'StarterBlog_Preview_Config',
				array(
	...
	/**
	 * Get all customizer settings/control that added via `starterblog/customizer/config` hook
	 *
	...
	static function get_config( $wp_customize = null ) {
		if ( is_null( self::$config ) ) {
			...
			$_config = apply_filters( 'starterblog/customizer/config', array(), $wp_customize );
			$config  = array();
			foreach ( $_config as $f ) {
				...
				$f = wp_parse_args(
					...
					array(
						 ...
						 * For settings
						 */
						'sanitize_callback'    => array( 'StarterBlog_Sanitize_Input', 'sanitize_customizer_input' ),
	...
	function get_setting( $name, $device = 'desktop', $key = false ) {
		...
		if ( isset( $config[ 'setting|' . $name ] ) ) {
			...
			$default = apply_filters( 'starterblog/customize/settings-default', $default, $name );
	...
	function get_typo_fields() {
		$typo_fields = array(
			array(
				'name'    => 'font',
				'type'    => 'select',
				'label'   => __( 'Font Family', 'starter-blog' ),
				'choices' => array(),
			),
			array(
				'name'    => 'font_weight',
				'type'    => 'select',
				'label'   => __( 'Font Weight', 'starter-blog' ),
				'choices' => array(),
			),
			array(
				'name'  => 'languages',
				'type'  => 'checkboxes',
				'label' => __( 'Font Languages', 'starter-blog' ),
			),
			array(
				'name'            => 'font_size',
				'type'            => 'slider',
				'label'           => __( 'Font Size', 'starter-blog' ),
			...
			array(
				'name'            => 'line_height',
				'type'            => 'slider',
				'label'           => __( 'Line Height', 'starter-blog' ),
				'device_settings' => true,
				'min'             => 9,
				'max'             => 80,
				'step'            => 1,
			...
			array(
				'name'  => 'letter_spacing',
				'type'  => 'slider',
				'label' => __( 'Letter Spacing', 'starter-blog' ),
				'min'   => - 10,
				'max'   => 10,
				'step'  => 0.1,
			),
			array(
				'name'    => 'style',
				'type'    => 'select',
				'label'   => __( 'Font Style', 'starter-blog' ),
				'choices' => array(
					''        => __( 'Default', 'starter-blog' ),
					'normal'  => __( 'Normal', 'starter-blog' ),
					'italic'  => __( 'Italic', 'starter-blog' ),
					'oblique' => __( 'Oblique', 'starter-blog' ),
				),
			),
			array(
				'name'    => 'text_decoration',
				'type'    => 'select',
				'label'   => __( 'Text Decoration', 'starter-blog' ),
				'choices' => array(
					''             => __( 'Default', 'starter-blog' ),
					'underline'    => __( 'Underline', 'starter-blog' ),
					'overline'     => __( 'Overline', 'starter-blog' ),
					'line-through' => __( 'Line through', 'starter-blog' ),
			...
			array(
				'name'    => 'text_transform',
				'type'    => 'select',
				'label'   => __( 'Text Transform', 'starter-blog' ),
				'choices' => array(
					''           => __( 'Default', 'starter-blog' ),
					'uppercase'  => __( 'Uppercase', 'starter-blog' ),
					'lowercase'  => __( 'Lowercase', 'starter-blog' ),
					'capitalize' => __( 'Capitalize', 'starter-blog' ),
					'none'       => __( 'None', 'starter-blog' ),
				),
			),
		);
	...
	function get_styling_config() {
		$fields = array(
			'tabs'          => array(
				'normal' => __( 'Normal', 'starter-blog' ),  // null or false to disable.
				'hover'  => __( 'Hover', 'starter-blog' ), // null or false to disable.
			),
			'normal_fields' => array(
				array(
					'name'       => 'text_color',
					'type'       => 'color',
					'label'      => __( 'Color', 'starter-blog' ),
					'css_format' => 'color: {{value}}; text-decoration-color: {{value}};',
				),
				array(
					'name'       => 'link_color',
					'type'       => 'color',
					'label'      => __( 'Link Color', 'starter-blog' ),
					'css_format' => 'color: {{value}}; text-decoration-color: {{value}};',
				),
				...
				array(
					'name'  => 'bg_heading',
					'type'  => 'heading',
					'label' => __( 'Background', 'starter-blog' ),
				),
				...
				array(
					'name'       => 'box_shadow',
					'type'       => 'shadow',
					'label'      => __( 'Box Shadow', 'starter-blog' ),
		...
		return apply_filters( 'starterblog/get_styling_config', $fields );
	}
	...
	function load_controls() {
		...
		$fields = apply_filters( 'starterblog/customize/register-controls', $fields );
		...
		foreach ( $fields as $k => $f ) {
			...
			if ( file_exists( $file ) ) {
				...
				$control_class_name = 'StarterBlog_Customizer_Control_';
	...
	function register( $wp_customize ) {
		...
		// Custom panel.
		require_once get_template_directory() . '/inc/customizer/class-starterblog-panel.php';
		// Load custom section.
		require_once get_template_directory() . '/inc/customizer/class-starterblog-section.php';

		// Load custom section pro.
		require_once get_template_directory() . '/inc/customizer/class-starterblog-section-pro.php';

		// Register new panel and section type.
		$wp_customize->register_panel_type( 'StarterBlog_WP_Customize_Panel' );
		$wp_customize->register_section_type( 'StarterBlog_WP_Customize_Section' );

		// Register section type.
		$wp_customize->register_section_type( 'StarterBlog_WP_Customize_Section_Pro' );
		...
		foreach ( self::get_config( $wp_customize ) as $args ) {
			switch ( $args['type'] ) {
				case 'panel':
					...
					unset( $args['name'] );
					unset( $args['type'] );
					$panel = new StarterBlog_WP_Customize_Panel( $wp_customize, $name, $args );
				...
				case 'section':
					...
					if ( isset( $args['section_class'] ) && class_exists( $args['section_class'] ) ) { // Allow custom class.
						...
						$wp_customize->add_section( new StarterBlog_WP_Customize_Section( $wp_customize, $name, $args ) );
					}
				...
				default:
					...
					$settings_args['default'] = apply_filters( 'starterblog/customize/settings-default', $args['default'], $args['name'] );

					$settings_args['transport'] = 'refresh';
					if ( ! $settings_args['sanitize_callback'] ) {
						$settings_args['sanitize_callback'] = array(
							'StarterBlog_Sanitize_Input',
					...
					if ( 'js_raw' != $settings_args['type'] ) {
						if ( class_exists( $control_class_name ) ) {
							...
							$wp_customize->add_control( new StarterBlog_Customizer_Control_Base( $wp_customize, $name, $args ) );
						}
					}
					...
					if ( $selective_refresh ) {
						...
						if ( ! isset( $this->selective_settings[ $__id ] ) ) {
							$this->selective_settings[ $__id ] = array(
								...
								'container_inclusive' => ( strpos( $__id, 'StarterBlog_Customizer_Auto_CSS' ) === false ) ? true : false,
		...
		$wp_customize->get_section( 'title_tagline' )->title        = __( 'Logo & Site Identity', 'starter-blog' );
		...
		foreach ( $this->selective_settings as $cb => $settings ) {
			...
			if ( 'StarterBlog_Builder_Item_Logo__render' == $cb ) {
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => array( 'StarterBlog_Sanitize_Input', 'sanitize_css_code' ),
			)
		);

		do_action( 'starterblog/customize/register_completed', $this );
	}
