...
class StarterBlog_Builder_Item_Nav_Icon {
	public $id = 'nav-icon';
	public $section = 'header_menu_icon';

	function item() {
		return array(
			'name'    => __( 'Menu Icon', 'starter-blog' ),
			'id'      => $this->id,
			'width'   => '3',
	...
	function customize() {
		...
		$config   = array(
			array(
				...
				'theme_supports' => '',
				'title'          => __( 'Menu Icon', 'starter-blog' ),
			),
			...
			array(
				...
				'selector'        => $selector,
				'render_callback' => $fn,
				'title'           => __( 'Label Settings', 'starter-blog' ),
			...
			array(
				...
				'device_settings' => true,
				'choices'         => array(
					'small'  => __( 'Small', 'starter-blog' ),
					'medium' => __( 'Medium', 'starter-blog' ),
					'large'  => __( 'Large', 'starter-blog' ),
				),
			),
			...
			array(
				...
				'title'      => __( 'Color Hover', 'starter-blog' ),
			),
		);
	...
	function render() {
		$label      = sanitize_text_field( StarterBlog()->get_setting( 'nav_icon_text' ) );
		$show_label = StarterBlog()->get_setting( 'nav_icon_show_text', 'all' );
		$style      = sanitize_text_field( StarterBlog()->get_setting( 'nav_icon_style' ) );
		$sizes      = StarterBlog()->get_setting( 'nav_icon_size', 'all' );
...
StarterBlog_Customize_Layout_Builder()->register_item( 'header', new StarterBlog_Builder_Item_Nav_Icon() );
