...
class StarterBlog_Builder_Header extends StarterBlog_Customize_Builder_Panel {
	public $id = 'header';
	...
	function get_config() {
		return array(
			'id'            => $this->id,
			'title'         => __( 'Header Builder', 'starter-blog' ),
			...
			'versions'      => array(
				'v1' => array(
					...
					'label' => __( 'Version 1', 'starter-blog' ),
				),
				'v2' => array(
					'control_id' => 'header_builder_panel_v2',
					'label' => __( 'Version 2', 'starter-blog' ),
				),
			),
			'devices'       => array(
				'desktop'      => __( 'Desktop', 'starter-blog' ),
				'mobile'       => __( 'Mobile/Tablet', 'starter-blog' ),
			),
		);
	}
	...
	function get_rows_config() {
		return array(
			'top'     => __( 'Header Top', 'starter-blog' ),
			'main'    => __( 'Header Main', 'starter-blog' ),
			'bottom'  => __( 'Header Bottom', 'starter-blog' ),
			'sidebar' => __( 'Menu Sidebar', 'starter-blog' ),
		);
	}
	...
	function row_config( $section = false, $section_name = false ) {
		...
		if ( ! $section_name ) {
			$section_name = __( 'Header Top', 'starter-blog' );
		}
		...
		$config = array(
			...
			array(
				...
				'max'             => 250,
				'selector'        => $selector . " .starterblog-grid, $selector .style-full-height .primary-menu-ul > li > a",
				'css_format'      => 'min-height: {{value}};',
				'title'           => __( 'Height', 'starter-blog' ),
			),
			...
			array(
				...
				'type'             => 'styling',
				'section'          => $section,
				'title'            => __( 'Advanced Styling', 'starter-blog' ),
				'description'      => sprintf( __( 'Advanced styling for %s', 'starter-blog' ), $section_name ),
	...
	function row_sidebar_config( $section, $section_name ) {
		...
		$config = array(
			...
			array(
				...
				'section'         => $section,
				'selector'        => 'body',
				'render_callback' => 'starterblog_customize_render_header',
				'css_format'      => 'html_class',
				'title'           => __( 'Display Type', 'starter-blog' ),
				'default'         => 'menu_sidebar_slide_left',
				'choices'         => array(
					'menu_sidebar_slide_left'    => __( 'Slide From Left', 'starter-blog' ),
					'menu_sidebar_slide_right'   => __( 'Slide From Right', 'starter-blog' ),
					'menu_sidebar_slide_overlay' => __( 'Full-screen Overlay', 'starter-blog' ),
					'menu_sidebar_dropdown'      => __( 'Toggle Dropdown', 'starter-blog' ),
				),
			),
			...
			array(
				...
				'checkbox_label' => __( 'Do not copy parent menu to submenu.', 'starter-blog' ),
				'default'        => 1,
			),
...
if ( ! function_exists( 'starterblog_header_layout_settings' ) ) {
	function starterblog_header_layout_settings( $item_id = '', $section = '', $cb = '', $name_prefix = 'header_' ) {

		if ( ! $cb ) {
			$cb = 'starterblog_customize_render_header';
		}
		...
		if ( ! $name_prefix ) {
			...
			if ( strpos( $item_id, 'footer' ) !== false ) {
				$class       = '.footer--row';
				$name_prefix = 'footer_';
				$cb          = 'starterblog_customize_render_footer';
			}
		}
		...
		$layout = array(
			array(
				...
				'priority' => 800,
				'section'  => $section,
				'title'    => __( 'Item Layout', 'starter-blog' ),
			),
			...
			array(
				...
				'title'           => __( 'Merge Item', 'starter-blog' ),
				'description'     => __( 'If you choose to merge this item, the alignment setting will inherit from the item you are merging.', 'starter-blog' ),
				'choices'         => array(
					0      => __( 'No', 'starter-blog' ),
					'prev' => __( 'Merge with left item', 'starter-blog' ),
					'next' => __( 'Merge with right item', 'starter-blog' ),
				),
				'required' => array( 'header_builder_version', '!=', 'v2' ),
			),
		);
...
StarterBlog_Customize_Layout_Builder()->register_builder( 'header', new StarterBlog_Builder_Header() );
